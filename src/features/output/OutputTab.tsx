...
export function logText(entries: GitLogEntry[]): string {
  ...
  for (const entry of entries) {
    ...
    if (stderr) lines.push(`${time} [info] ${stderr}`)
  }
  return lines.join('\n')
}
...
function useLogText(failure: string | null, legacy: boolean): string | null {
  const client = useQueryClient()
  // The log is read fresh on every mount and again after each new command, so it never shows
  // a copy from while the tab was closed
  const log = useQuery({ queryKey: LOG_KEY, queryFn: ipc.gitLogEntries, staleTime: 0, refetchOnMount: 'always', enabled: !failure && !legacy })
  useTauriEvent<GitLogEntry>('git-log://entry', () => {
    if (!failure) void client.invalidateQueries({ queryKey: LOG_KEY })
  })
  ...
  if (failure) return kept.isPending ? null : kept.data ? commandText(kept.data) : t('output.gone')
  return log.data ? logText(log.data) : null
}
...
export function OutputTab({ params }: DetailTabProps) {
  ...
  const text = useLogText(failure, legacy) ?? ''
