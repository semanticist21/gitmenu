...
fn validate(raw: &str) -> Option<String> {
    ...
    let first = lines.next()?.trim().trim_start_matches('#').trim().replace(['<', '>'], "");
    ...
    let body: Vec<&str> = lines.map(str::trim_end).skip_while(|l| l.trim().is_empty()).collect();
    let first = format!("{kind}{scope}: {summary}");
    Some(if body.is_empty() { first } else { format!("{first}\n\n{}", body.join("\n")) })
}
...
mod tests {
    ...
    fn validates_conventional_commits() {
        assert_eq!(validate("feat(scm): add amend button").as_deref(), Some("feat(scm): add amend button"));
        ...
        assert_eq!(validate("## fix(queue): 재시도 추가").as_deref(), Some("fix(queue): 재시도 추가"));
        assert_eq!(validate("<feat>(add): Adds retries").as_deref(), Some("feat(add): adds retries"));
