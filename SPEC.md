...
- **탭 유지**: 상세 창의 열린 탭 편집기는 숨긴 채 마운트를 유지한다(VS Code가 편집기를 그대로 두듯). 다시 보면 데이터·하이라이트·선택·스크롤이 그대로라 다시 읽지 않는다. 저장소 변경·쓰기 완료 이벤트는 숨긴 탭에도 반영하되, 숨긴 탭은 전역 명령과 컨텍스트 키를 가져가지 않는다. 최근 8개만 유지하고 초과한 가장 오래된 탭은 탭을 닫지 않고 언마운트한다(다시 보면 그때 읽는다). 터미널 탭만은 화면을 세션이 들고 있으므로(위 통합 터미널 항목) 활성일 때만 마운트한다.
